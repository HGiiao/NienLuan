...
import { CheckCircle, XCircle, Loader, Home, Ticket, ArrowRight } from 'lucide-react'
...
export default function VnPayReturn() {
  ...
  useEffect(() => {
    const verify = async () => {
      try {
        // Collect all query params from VNPay return
        const params = {}
        ...
        const res = await api.post('/api/payments/vnpay-return', params)
        const data = res.data
        ...
        if (data.success) {
          ...
          try {
            const bookingRes = await getBooking(params.vnp_TxnRef)
      ...
      >
        ...
        {transactionId && (
          <div className="flex items-center justify-center gap-1.5 text-xs text-[var(--color-text-tertiary)] mb-4">
            ...
            Mã giao dịch VNPay: <span className="font-mono font-semibold text-[var(--color-text-secondary)]">{transactionId}</span>
          </div>
        )}
