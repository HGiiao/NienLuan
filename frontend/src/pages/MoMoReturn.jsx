...
import { CheckCircle, XCircle, Loader, Home, Ticket, Shield } from 'lucide-react'
import { getBooking } from '../services/api'
import { formatCurrencyVnd } from '../utils/formatters'
import api from '../services/api'

function parseBookingId(orderId) {
  const digits = (orderId || '').match(/^\d+/)
  ...
  useEffect(() => {
    const verify = async () => {
      try {
        ...
        const parsedId = parseBookingId(orderId)
        if (!parsedId) {
        ...
        const res = await api.post('/api/payments/momo-return', {
          orderId,
          resultCode,
          transId,
        })
      ...
      >
        ...
        {transactionId && (
          <div className="flex items-center justify-center gap-1.5 text-xs text-[var(--color-text-tertiary)] mb-4">
            ...
            Mã giao dịch MoMo: <span className="font-mono font-semibold text-[var(--color-text-secondary)]">{transactionId}</span>
          </div>
        )}
