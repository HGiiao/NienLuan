...
export default function PayOSReturn() {
  ...
  useEffect(() => {
    const verify = async () => {
      try {
        // Collect all query params from PayOS return
        const params = {}
        ...
        setBookingId(params.orderCode)

        // Send to backend for verification (authoritative via PayOS API)
        const res = await api.post('/api/payments/payos-return', params)
        const data = res.data
      ...
      >
        ...
        {transactionId && (
          <div className="flex items-center justify-center gap-1.5 text-xs text-[var(--color-text-tertiary)] mb-4">
            ...
            Mã giao dịch PayOS: <span className="font-mono font-semibold text-[var(--color-text-secondary)]">{transactionId}</span>
          </div>
        )}
