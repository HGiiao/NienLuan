...
export default function TrainCard({ train, onBook, onWatch, onDetail, badge, index = 0, prediction, rating, watched = false }) {
  ...
  const seatsLeft = train.seats
  const isLowStock = seatsLeft <= 5
  const hasDeparted = new Date(train.departureTime) <= new Date()
  const { avgPrice, vsAverage, trend } = useMemo(() => {
    const seed = (train.id * 9301 + 49297) % 233280
    const r = seed / 233280
    const avg = train.price * (1 + (r * 0.25 - 0.125))
    const vs = ((train.price - avg) / avg * 100)
        ...
        <div className="hidden md:flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-bold text-[var(--color-text-primary)]">{train.trainCode}</div>
            ...
            <div className="text-right">
              <div className="text-[26px] font-black text-primary-500 leading-none tracking-tight">{formatCurrencyVnd(train.price)}</div>
          ...
          <div className="flex items-center gap-3">
            <div className="text-right min-w-[72px]">
              <div className="text-[22px] font-bold text-[var(--color-text-primary)] leading-none tracking-tight">{fmt(train.departureTime)}</div>
              <div className="text-[13px] font-semibold text-[var(--color-text-secondary)] mt-0.5">{train.departureLocation}</div>
              <div className="text-[11px] text-[var(--color-text-tertiary)]">Ga đi</div>
            </div>
            <div className="flex-1 flex flex-col items-center px-2">
                ...
                <div className="flex-1 h-[2px] bg-[var(--color-border)] relative">
                  ...
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                    <div className="w-5 h-5 rounded-full flex items-center justify-center bg-primary-500/15 border border-primary-500/20 shadow-sm">
                      <Train className="w-2.5 h-2.5 text-primary-500" />
                    </div>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-1.5 mt-2">
                <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-primary-500 bg-primary-500/10 px-2 py-0.5 rounded-full border border-primary-500/20">
                  <Train className="w-3 h-3" />{train.trainName || 'Tàu khách'}
                </span>
              </div>
            </div>
            <div className="text-left min-w-[72px]">
              <div className="text-[22px] font-bold text-[var(--color-text-primary)] leading-none tracking-tight">{fmt(train.arrivalTime)}</div>
              <div className="text-[13px] font-semibold text-[var(--color-text-secondary)] mt-0.5">{train.arrivalLocation}</div>
              <div className="text-[11px] text-[var(--color-text-tertiary)]">Ga đến</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-[var(--color-border)]">
            <div className="flex flex-wrap items-center gap-1.5 min-w-0">
              ...
              {badge && (
                <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-0.5 rounded-full border bg-primary-500/10 text-primary-500 border-primary-500/20">
                  <TrendingDown className="w-3 h-3" />{badge}
                </span>
              )}
            ...
            <div className="flex items-center gap-2 shrink-0 ml-auto">
              {onWatch && (
                <motion.button whileHover={hasDeparted ? undefined : { scale: 1.03 }} whileTap={hasDeparted ? undefined : { scale: 0.97 }}
                  ...
                  onClick={() => { if (hasDeparted) return; onWatch?.(train) }}
              ...
              <motion.button whileHover={hasDeparted ? undefined : { scale: 1.03 }} whileTap={hasDeparted ? undefined : { scale: 0.97 }}
                disabled={hasDeparted}
                onClick={() => { if (hasDeparted) return; onBook?.(train) }}
                className={hasDeparted
        ...
        <div className="md:hidden p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center text-white shadow-sm"><Train className="w-5 h-5" /></div>
              <div><div className="text-sm font-bold text-[var(--color-text-primary)]">{train.trainCode}</div><div className="text-[11px] text-[var(--color-text-tertiary)]">{train.trainName || `Hạng ${train.coachClass || 'Ngồi mềm'}`}</div></div>
            </div>
            <div className="text-right"><div className="text-xl font-black text-primary-500">{formatCurrencyVnd(train.price)}</div><div className="text-[10px] text-[var(--color-text-tertiary)]">Đã bao gồm thuế</div></div>
          </div>
          <div className="flex items-center gap-2 py-1">
            <div className="flex-1"><div className="text-lg font-bold text-[var(--color-text-primary)]">{fmt(train.departureTime)}</div><div className="text-xs font-semibold text-[var(--color-text-secondary)]">{train.departureLocation}</div></div>
            ...
            <div className="flex-1 text-right"><div className="text-lg font-bold text-[var(--color-text-primary)]">{fmt(train.arrivalTime)}</div><div className="text-xs font-semibold text-[var(--color-text-secondary)]">{train.arrivalLocation}</div></div>
          </div>
          <div className="flex items-center gap-2 pt-1">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {badge && <span className="text-[10px] font-bold text-primary-500 bg-primary-500/10 px-2 py-0.5 rounded-full border border-primary-500/20">🏷️ {badge}</span>}
          ...
          <div className="flex items-center justify-between text-[11px] text-[var(--color-text-tertiary)] pt-1 border-t border-[var(--color-border)]">
            <div className="flex items-center gap-2">
              <Clock className="w-3 h-3" /><span>{train.coachClass || 'Ngồi mềm'}</span>
